...
const Page = () => {
    ...
    const { products, status: productStatus, error: productError } = useAppSelector((state: RootState) => state.products);
    const { cart, status: cartStatus, error: cartError } = useAppSelector((state: RootState) => state.cart);
    ...
    return (
        <div className="mt-20 text-center text-2xl">
            ...
            <div>
                ...
                {productStatus === "failed" && productError && <p>{currentTexts.errorLoadingProducts.replace("{error}", productError)}</p>}
            ...
            <div className="mt-8">
                ...
                {cartStatus === "failed" && cartError && <p>{currentTexts.errorLoadingCart.replace("{error}", cartError)}</p>}
                {cartStatus === "succeeded" && cart?.items?.length ? (
                    <div>
                        <h3 className="text-2xl font-semibold">{currentTexts.yourCart}</h3>
                        <div className="mt-4">
                            <h4 className="text-lg font-bold">{currentTexts.totalCartPrice.replace("{price}", cart.totalPrice.toString())}</h4>
                        </div>
                    </div>
                ) : (
