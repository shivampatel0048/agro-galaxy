...
const Navbar = () => {
    ...
    useEffect(() => {
        if (isMenuOpen) {
            animate([
                [".mobile-menu", { opacity: [0, 1], y: ["-100%", "0%"] }, { duration: 0.3 }],
                ["a", { opacity: [0, 1], y: [20, 0] }, { duration: 0.3, delay: stagger(0.1) }],
    ...
    return (
        <div className="bg-white">
            <nav className="fixed top-0 left-0 right-0 bg-white/80 backdrop-blur-sm border-b border-gray-200 z-50">
                ...
                <div
                    ref={scope}
                    className={`mobile-menu md:hidden fixed inset-0 bg-white/95 backdrop-blur-sm z-40 ${isMenuOpen ? "opacity-100 w-full min-h-screen" : "opacity-0 pointer-events-none"
                        }`}
                >
